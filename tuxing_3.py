...
def init_para():
    global value,myresult,try_queue
    value=[]      #转换后的值
    myresult=[]  #输出数组
...
def value_valid(mycol,myrow,my_value):
    mycol_list=[value[mycol][1][1], value[mycol][2][1], value[mycol][3][1], value[mycol][4][1], value[mycol][5][1],value[mycol][6][1], value[mycol][7][1], value[mycol][8][1], value[mycol][0][1]]
    myrow_list=[value[0][myrow][1], value[1][myrow][1], value[2][myrow][1], value[3][myrow][1], value[4][myrow][1],value[5][myrow][1], value[6][myrow][1], value[7][myrow][1], value[8][myrow][1]]
    my_i = (mycol) // 3 * 3
    my_j = (myrow) // 3 * 3
    myblock_list=[value[my_i][my_j][1], value[my_i][my_j + 1][1], value[my_i][my_j + 2][1], value[my_i + 1][my_j][1],value[my_i + 1][my_j + 1][1], value[my_i + 1][my_j + 2][1], value[my_i + 2][my_j][1],value[my_i + 2][my_j + 1][1], value[my_i + 2][my_j + 2][1]]
    mycol_list.remove(value[mycol][myrow][1])
    myrow_list.remove(value[mycol][myrow][1])
    myblock_list.remove(value[mycol][myrow][1])
    if my_value not in mycol_list:
        if my_value not in myrow_list:
            if my_value not in myblock_list:
                return my_value
    return 0
